...
本仓库同级目录包含配套前端项目 `ppt-speech-nextjs/`，用于语音选择、上传任务、SSE 实时进度与结果下载。

本地联调建议：

1. 先启动本服务端（本项目 README 其余部分为服务端使用说明）
2. 再启动前端并通过环境变量 `BACKEND_ORIGIN` 配置后端地址（默认 `http://127.0.0.1:8000`）
